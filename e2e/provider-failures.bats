...
request() { curl --silent --show-error --output - --write-out $'\n%{http_code}' --get "$COMPANY_CHECK_API_URL/backend-service" --data-urlencode "verificationId=$1" --data-urlencode "query=$2"; }

@test "FREE no-result and 503 use PREMIUM fallback" { result="$(request "$id" fallback)"; [[ "$result" == *'"status":"MATCH"'* ]]; }
@test "provider 4xx is terminal and does not fallback" { result="$(request "$id" client-error)"; [[ "$result" == *PROVIDER_CLIENT_ERROR* ]]; [[ "$result" == *$'\n502' ]]; }
@test "malformed FREE payload falls back to PREMIUM" { result="$(request "$id" malformed)"; [[ "$result" == *'"status":"MATCH"'* ]]; }
